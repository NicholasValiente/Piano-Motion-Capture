var game = 3; //the map scenario to load
var time = 1.5; //game length (in minutes)
var numRings = 50; //the number of rings to place betwen each placemark
var variance = 0; //the randomness at whcih to place the rings(between 1 and 10 works best)
...
/*Placemark Locations*/
var harbour = [-33.875716,151.201699];
var bridge = [-33.852045,151.210817];
...
{
	for (a=2;a<audiochannels.length;a++)  //hardcoded to start from 2; to allow music to take 0 and flight to use 1
	{
		thistime = new Date();
...
function initCB(instance) {
    ...
    camera.setAltitudeMode(ge.ALTITUDE_RELATIVE_TO_GROUND);
...
function spaceRings(sLat, sLon, fLat, fLon, noRings, passedLat, passedLon, passedAlt, m) //takes in 2 coordinate points and allocates the number of spots 'norings' between them evenly. It then stores thes points in lat and lon
{
	...
	{
		for (t=0; t<=2;t++){
			randNum[t] = gRand(variance);}//generate random numbers for each ring location (used to randomize positions slightly)
		
		passedAlt[m] = (500 + randNum[0]);
...
function setupGame()
{
	...
	for (i=0; i<totRings-1; i++)
	{drawRings(i, 'blue');}	//draw all rings but last blue
	drawRings(totRings-1, 'red');//draw final ring red
}
	
function updateRings()
{
	var m;
	for (m=0;m<totRings;m++)
	{
		if (ring[m].colour == 'blue')
	}
}
	
function drawRings(i, colour) 
{
...
function checkIncrementScore()	//check to see if the player is pasing a ring
{
	var m;
	if (start == 0)
	{
	...
	{
	playSounds(0, 'multiaudio1'); //play the music file on channel 0
	}
	else
	{
	playSounds(0, 'multiaudio3'); //play the end game music file on channel 0	
	}
	
	for (m=0;m<totRings;m++)	//for each ring
	{	
		if (passed[m] == false)	//if the ring hasent yet been passed
		{
			...
			&& ((camera.getAltitude() > (alt[m] -(5*sVals[2]*3))) && (camera.getAltitude() < (alt[m] +(5*sVals[2])))))
				{
				play_multi_sound('targetaudio'+ ((4)));	//play the closeness sound
