...
#
# run with 
# perl websocket-echo.pl daemon --listen http://*:3000
# or simply
# perl websocket-echo.pl
...
websocket '/relay' => sub {		# ws://IP_ADDRESS:3000/relay
  ...
  $self->on(message => sub {
	...
	if ($message eq "master") {	# remove a 'master' from client send list
...
# app->secret('wonderama');		# deprecated mojo cookie (just a word)
app->start;
